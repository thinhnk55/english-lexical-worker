...
export async function requireAdmin(request: Request, env: Env, origin: string): Promise<AuthResult> {
  ...
  if (!result.valid || !result.payload) return { ok: false, response: errorResponse(401, 'UNAUTHORIZED', undefined, origin) };
  if (result.payload.role !== 'admin' && result.payload.role !== 'super_admin') {
